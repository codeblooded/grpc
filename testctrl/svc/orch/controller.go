...
func (c *Controller) Schedule(s *types.Session) error {
	...
	glog.Warningf("enqueued session %v", s)
	return nil
}
...
func (c *Controller) loop() {
	for {
		session, quit := c.next()
		glog.Warningf("session %v quit %v", session, quit)
		if quit {
			return
		}
...
func (c *Controller) next() (session *types.Session, quit bool) {
	...
	if !c.running {
		glog.Warningf("not running")
		return nil, true
	}

	if c.activeCount > executorCount {
		glog.Warningf("not enough executors")
		return nil, false
	}

	glog.Warningf("dequeued session")

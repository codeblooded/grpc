...
func TestControllerSchedule(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.description, func(t *testing.T) {
			...
			err := controller.Schedule(c.session)
			if c.shouldError && err == nil {
				...
				if got == nil {
					t.Fatalf("expected executor to receive session %v, but it did not", c.session.Name)
				}
				if got.Name != c.session.Name {
					t.Fatalf("expected executor to receive session %v, but got %v", c.session.Name, got.Name)
				}
			}
		})
	}
...
func TestControllerStart(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			mockNL:      &nodeListerMock{Error: errors.New("fake kubernetes error")},
			shouldError: true,
		},
		{
	...
	for _, c := range cases {
		t.Run(c.description, func(t *testing.T) {
			...
			if c.mockNL != nil {
				controller.nl = c.mockNL
			}

			if c.mockPW != nil {
				controller.pw = c.mockPW
				controller.watcher = NewWatcher(c.mockPW)
			}
	...
	for _, tc := range cases {
		t.Run(tc.description, func(t *testing.T) {
			...
			if tc.shouldError && err == nil {
				t.Errorf("executors unexpectedly finished before timeout was reached")
			} else if !tc.shouldError && err != nil {
				t.Errorf("timeout unexpectedly reached, executors not finished")
			}
